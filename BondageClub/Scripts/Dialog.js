...
function DialogMenuButtonBuild(C) {
	...
	if ((DialogProgress < 0) && !DialogActivityMode) {
		...
		if ((Item != null) && Item.Asset.ExtendedOrTypeInfo && Player.CanInteract() && !IsGroupBlocked && (!Item.Asset.OwnerOnly || (C.IsOwnedByPlayer())) && (!Item.Asset.LoverOnly || (C.IsLoverOfPlayer()))) DialogMenuButton.push("Use");
...
function DialogDrawItemMenu(C) {
	...
	if (DialogProgress >= 0) {
		...
		if (DialogProgress >= 100) {
			...
			if (DialogProgressNextItem != null) InventoryWear(C, DialogProgressNextItem.Asset.Name, DialogProgressNextItem.Asset.Group.Name, (DialogColorSelect == null) ? "Default" : DialogColorSelect, SkillGetLevel(Player, "Bondage"), DialogProgressNextItem.Property);
