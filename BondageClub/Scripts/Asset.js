...
function AssetAdd(NewAsset, Group) {
	var A = {
		Name: NewAsset.Name,
		...
		IsRestraint: (NewAsset.IsRestraint == null) ? ((Group.IsRestraint == null) ? false : Group.IsRestraint) : NewAsset.IsRestraint,
	...
	for (G = 0; G < A.length; G++) {
		...
		for (I = 0; I < A[G].Asset.length; I++)
			if (A[G].Asset[I].Name == null)
				AssetAdd({ Name: A[G].Asset[I] }, A[G]);
			else
				AssetAdd(A[G].Asset[I], A[G]);
